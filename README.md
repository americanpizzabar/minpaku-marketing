...
AirROIはPay-per-call（検索 $0.50/回、料金カレンダー $0.10/回）のため、同期は増分方式です。

- **物件カタログ**（エリア検索）: `AIRROI_SEARCH_REFRESH_DAYS`（既定30日）ごとにのみ再検索
- **料金カレンダー**: 各物件を `AIRROI_RATES_REFRESH_DAYS`（既定14日）周期で更新。1回の同期で取得するのは古い順に `AIRROI_DAILY_RATES_CALLS`（既定40）件まで
- 全件再取得は `GET /api/sync?key=<CRON_SECRET>&force=1`（データ全消去は `reset=1`）。上限なしで実行されるためコストに注意

**月額コストの目安**（追跡 約500物件の場合）:

| 料金更新周期 | 日次コスト | 月額（カタログ$25込み） |
|---|---|---|
| 7日 | ~$7.2 | ~$240 |
| **14日（既定）** | **~$3.6** | **~$130** |
| 30日 | ~$1.7 | ~$75 |

計算式: `追跡物件数 × $0.10 ÷ 更新周期日 = 日次コスト`。物件数を減らす（`AIRROI_MAX_LISTINGS_PER_AREA`）と比例して下がります。
